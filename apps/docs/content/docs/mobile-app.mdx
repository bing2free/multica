...
Release builds bake the API address into the app. `apps/mobile/.env.production` is committed with the public multica.ai defaults, so a fresh clone builds against production as-is.

To connect to a self-hosted instance, copy `apps/mobile/.env.production.example` to `apps/mobile/.env.production.local` and edit the copy:
...
The `.local` file overrides the committed defaults and is gitignored, so personal endpoints never reach the repository. `EXPO_PUBLIC_API_URL` is required. `EXPO_PUBLIC_WEB_URL` is optional and powers entries like "Copy link" and "Open on web"; when it's unset, those menu items are hidden.
...
Editing `.env.production` directly changes the production configuration of the current checkout. Keep personal values in the gitignored `.env.production.local`.
</Callout>
